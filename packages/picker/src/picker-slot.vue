<template>
  <div class="picker-slot" :class="classNames" :style="flexStyle">
    <div v-if="!divider" ref="wrapper" class="picker-slot-wrapper" :class="{ dragging: dragging }" :style="{ height: contentHeight + 'px' }">
      <div class="picker-item" v-for="itemValue in mutatingValues" :key="itemValue[valueKey] || itemValue" :class="{ 'picker-selected': itemValue === currentValue || (typeof itemValue === 'object' && itemValue[valueKey] ? itemValue[valueKey] === currentValue[valueKey] : false)}" :style="{ height: itemHeight + 'px', lineHeight: itemHeight + 'px' }">
